...
class AllEmployees extends React.Component {
        ...
        ref.on('value', snapshot => {
            ...
            snapshot.forEach(childSnapShot => {
                ...
                const GSTData = {
                    ...
                    user_name: childSnapShot.val().user_name,
                    ...
                    email_id: childSnapShot.val().email_id,
    ...
    employeemailChange  = (e) => {
        ...
        if(this.state.validError!=true){
            ...
            var ref = firebase
            .database()
            .ref('merchant_users/').orderByChild("email_id").equalTo(e.target.value);
            ref.on('value', snapshot => {
    ...
    <button type="button"  data-toggle="modal" data-target="#add_employee">
   ...
   <img src="/images/icon/add_plus_icon_w.svg"/> 
    ...
    <ul>
    <li><a href="/AllEmployees" className="activemenu">Employees</a></li>
    ...
    <tbody  id="myTable">
    {this.state.employeeList && this.state.employeeList.map((employee,index) => {
return (
...
<td>{index+1}</td>
    ...
    <td><img src={employee.photo} className="user_profile"/></td>
...
<h5 className="modal-title" id="smallmodalLabel">Add Employee 
</h5>
</div>
...
<input type="text" id="text-input" 
 ...
 onChange={this.onChange}
placeholder="" className="form-control edit_product"/>
{this .validator.message("UserName", this.state.user_name, "required|whitespace|min:2|max:70")}
</div>
</div>
</div>
...
placeholder="" className="form-control edit_product"/>
{this .validator.message("Mobile Number", this.state.contact_number, "required|whitespace|min:10|max:10")}
<div className="text-danger">
                                                        {" "}

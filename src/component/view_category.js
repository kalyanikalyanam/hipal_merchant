  ...
  itemCategoryList() {
    ...
    ref.on("value", (snapshot) => {
      ...
      snapshot.forEach((childSnapShot) => {
        const GSTData = {
          itemcategoryId: childSnapShot.key.toString(),
          item_category: childSnapShot.val().item_category,
    ...
    if (this.state.validError != true) {
      ...
      ref.on("value", (snapshot) => {
        ...
        if (user_exist > 0 && this.state.validError != true) {
          this.setState({
            mobile_message: "Item Category Name already exist",
            validError: false,
          });
        } else {

...
class AddItemMenu extends React.Component {
  ...
  itemTypeList() {
    ...
    ref.on("value", (snapshot) => {
      ...
      snapshot.forEach((childSnapShot) => {
        const GSTData = {
          itemtypeId: childSnapShot.key.toString(),
          item_type: childSnapShot.val().item_type,
  ...
  stationList() {
    ...
    ref.on("value", (snapshot) => {
      ...
      snapshot.forEach((childSnapShot) => {
        const GSTData = {
          stationId: childSnapShot.key.toString(),
          station_name: childSnapShot.val().station_name,
          businessId: childSnapShot.val().businessId,
          sessionId: childSnapShot.val().sessionId,
        };
  ...
  handleSubmit = (event) => {
    ...
    if (this.validator.allValid()) {
      ...
      var businessId = sessionStorage.getItem("businessId");
      let dbCon = firebase.database().ref("/merchant_menu_items");
...
class AddItemMenu extends React.Component {
  ...
  handleSubmit = async (event) => {
    ...
    if (this.validator.allValid()) {
      ...
      var username = sessionStorage.getItem("username");
      var businessId = sessionStorage.getItem("businessId");
      let dbCon = await firebase.firestore().collection("/menuitems");
      var key = Math.round(new Date().getTime() / 1000);
